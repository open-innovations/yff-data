...
import { basefs, mergeDeep, Series } from "./utils.js";
...
export function BarChart(config, csv) {
  ...
  const opt = {
    ...
    'buildSeries': function () {
      // Series
      let data, datum, label, i, s, categoryoffset, seriesoffset;
      for (s = 0; s < this.opt.series.length; s++) {
        ...
        if (this.opt.series[s].errors && this.opt.series[s].errors.length == 1)
          this.opt.series[s].errors.push(this.opt.series[s].errors[0]);
        data = [];
        for (i = 0; i < csv.rows.length; i++) {
          ...
          try {
            ...
            throw new Error(`Invalid category "${this.opt.category}" provided for Bar Chart built from "${this.opt.file}"`);
          }
          ...
          if (this.opt.series[s].errors)
            datum.error = { 'x': [csv.columns[this.opt.series[s].errors[0]][i], csv.columns[this.opt.series[s].errors[1]][i]] };
    ...
    'updatePadding': function () {
      let i, l, len, ax, lines, align;
      // Work out padding
      const pad = { 'l': 0, 't': 0, 'b': 0, 'r': 0 };
      for (ax in this.opt.axis) {
        ...
        for (l in this.opt.axis[ax].labels) {
          ...
          if (ax == "x") {
            if (align == "bottom")
              pad.b = Math.max(pad.b, len);
            else
              pad.t = Math.max(pad.t, len);
          } else {
            if (align == "left")
              pad.l = Math.max(pad.l, len);
            else
              pad.r = Math.max(pad.r, len);
          }
        }
      }
