...
function CS(grad) {
	...
	if(typeof grad==="string"){
		stops = extractColours(grad);
	}else if(typeof grad==="object" && grad.stops){
		for(var i = 0; i < grad.stops.length; i++){
			grad.stops[i].v = grad.stops[i].value;
			grad.stops[i].c = grad.stops[i].colour;
		}
		stops = grad.stops;
		quantised = true;
		// Check if a valid Math function is passed
		if(typeof grad.function==="string" && typeof Math[grad.function]==="function") method = grad.function;
		grad = "";
	}
	...
	function getColour(v, min=0, max=1){
		if(quantised){
			let range, v2, c;
			for(c = 0; c < stops.length-1; c++){
				...
				range = max - min;
				if(range <= 0) range = 1;
				v2 = min + Math[method]((v - min)/range)*range;
				// If it is in the range for this stop return the colour
				if(v2 >= min && v2 < max) return stops[c].c||'rgba(0,0,0,1)';
			}
			// Return the last stop
			return stops[stops.length-1].c||'rgba(0,0,0,1)';
		}
		var v2, pc, c, cfinal;
