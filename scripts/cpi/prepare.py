...
def summarise(metadata):
    ...
    latest['Note'] = [
        "As at {date}. This is {indicator} since the last update.".format(
            date=d, indicator=indicator.iloc[0, 0]),
        "As at {date}. This is {indicator} since the last update.".format(
            date=d, indicator=indicator.iloc[1, 0]),
        "As at {date}. This is {indicator} since the last update.".format(
            date=d, indicator=indicator.iloc[2, 0])
    ]
...
def bar_chart(data, n, make_indicator=True):
    ...
    df = pd.DataFrame()
    df['monthly_pct_change'] = pct_change(
        most_recent_month.value, last_month.value)
    df['quarterly_pct_change'] = pct_change(
        most_recent_month.value, last_quarter.value)
    df['yearly_pct_change'] = pct_change(
    ...
    if make_indicator == True:
        indicator = pd.DataFrame(
            data={'indicator': indicators, 'sign': sgn}, index=index)
...
def youth_average(data):
    data.loc['Youth-focused average',
             :] = data.iloc[1:].mean(axis=0, numeric_only=True)
    return data
...
if __name__ == '__main__':
    ...
    data = pd.read_csv(os.path.join(
        INPUTS_DIR, 'transformed_cpi.csv'), index_col='variable')
    ...
    summary_bar = bar.reindex(
        index=['All CPI Categories', 'Youth-focused average'])
